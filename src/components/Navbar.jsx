  ...
  return (
    <div>
        <nav className='fixed left-0 right-0 top-4 z-50'>
            <div className='mx-auto hidden max-w-2xl items-center justify-center rounded-lg bg-white/20 py-30 backdrop-blur-lg lg:flex'>
                    ...
                    <div>
                        <ul className='flex items-center gap-6'>
                            {NAVIGATION_LINKS.map ((item, index) =>(
                                <li key={index}>
                                    <a className='text-sm font-bold hover:text-yellow-400 size-4' href={item.href} onClick={(e) => handleLinkClick(e, 
                                        item.href)}>
                                            {item.label}
